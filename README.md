...
You can package this server into a standalone Windows executable (`.exe`) that includes a visual configuration tool.

### Building the Executable

1. Make sure you have installed the dependencies: `npm install`
2. Run the build command: `npm run dist:exe`
3. The executable will be generated in the `release/` folder.

### Using the Executable

**GUI Configuration Mode**
Simply double-click the `.exe` file. A configuration window will open where you can enter your SAP connection details. Clicking "Save" will store these credentials securely in `~/.mcp-abap-abap-adt-api/.env` and give you the exact JSON to paste into your MCP client config.

Alternatively, you can start the GUI configuration during development by running:
```bash
npm run electron:start
```

**MCP Server Mode**
To run the executable as an MCP server, pass the `--server` flag. Configure your MCP client (like Claude Desktop in `claude_desktop_config.json`) like this:

```json
{
  "mcpServers": {
    "mcp-abap": {
      "command": "C:\\path\\to\\your\\release\\mcp-abap-abap-adt-api.exe",
      "args": ["--server"]
    }
  }
}
